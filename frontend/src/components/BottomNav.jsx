...
const NavItem = ({ icon, label, to, isHiddenOnMobile = false }) => {
 ...
 const inactiveClasses = "text-tema-text-muted dark:text-tema-text-muted-dark hover:bg-gray-200 dark:hover:bg-gray-700";
...
const BottomNav = () => {
 const { userProfile, isLoggedIn, loadingProfile } = useAuth();
 const [isMenuOpen, setIsMenuOpen] = useState(false);

 // Se estiver carregando o perfil ou não estiver logado, não renderiza nada ainda
 if (loadingProfile || !isLoggedIn || !userProfile) {
    // console.log("BottomNav: Aguardando perfil ou login..."); // Log (opcional)
    return null;
 }

 // **REMOVIDO:** Lógica condicional removida
 ...
 return (
  <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-10">
   <div className="flex items-center gap-2 bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm p-2 rounded-full shadow-lg border dark:border-gray-700">
    ...
    {/* **AJUSTE:** Renderiza os ícones incondicionalmente */}
    <NavItem icon={<UserIcon />} label="Garçom" to="/mesas" />
    <NavItem icon={<DeliveryIcon />} label="Delivery" to="/delivery" />

    {/* Botões desktop */}
    <NavItem icon={<CalculatorIcon />} label="Caixa" to="/caixa" isHiddenOnMobile={true} />
    ...
    <div className="relative md:hidden">
     <button
      ...
      className="flex flex-col items-center justify-center p-2 rounded-full transition-all duration-300 min-w-[60px] text-tema-text-muted dark:text-tema-text-muted-dark hover:bg-gray-200 dark:hover:bg-gray-700"
     >
      <MoreIcon />
     ...
     {isMenuOpen && (
      <div className="absolute bottom-full mb-2 right-0 bg-white dark:bg-gray-800 rounded-lg shadow-xl border dark:border-gray-700 p-2 min-w-[150px] space-y-1">
       {/* **AJUSTE:** Renderiza os links incondicionalmente */}
       <Link to="/caixa" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 text-sm text-tema-text dark:text-tema-text-dark">
        <CalculatorIcon /> Caixa
       </Link>
       <Link to="/admin" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 text-sm text-tema-text dark:text-tema-text-dark">
        <SettingsIcon /> Admin
       </Link>
      </div>
     )}
