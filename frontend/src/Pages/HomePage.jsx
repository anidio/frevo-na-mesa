import React from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext.jsx';

const LampiaoHatIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M5 14h14a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2z" />
        <path d="M6 14c.8-3.33 3.33-5.83 6-6.5 2.67.67 5.17 3.17 6 6.5" />
        <path d="m10 10 1-1 1 1" />
        <path d="m14 10 1-1 1 1" />
    </svg>
);

const CactusIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 8v13" />
        <path d="M15.5 8a3.5 3.5 0 0 0-7 0v13" />
        <path d="M6 8a6 6 0 0 1 12 0" />
        <path d="M8 4a2 2 0 0 0-2-2h0a2 2 0 0 0-2 2v2" />
    </svg>
);

const SmartphoneIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <rect x="5" y="2" width="14" height="20" rx="2" ry="2"></rect>
        <line x1="12" y1="18" x2="12.01" y2="18"></line>
    </svg>
);

const ProfileCard = ({ title, description, features, icon, linkText, linkTo }) => (
    <div className="bg-white dark:bg-tema-surface-dark p-6 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105 border border-gray-200 dark:border-gray-700 flex flex-col">
        <div className="flex-grow">
            <div className="flex justify-center mb-4 text-tema-primary">{icon}</div>
            <h3 className="text-xl font-semibold text-center mb-2 text-tema-text dark:text-tema-text-dark">{title}</h3>
            <p className="text-sm text-center text-tema-text-muted dark:text-tema-text-muted-dark mb-4">{description}</p>
            <ul className="list-disc list-inside text-tema-text-muted dark:text-tema-text-muted-dark text-sm space-y-1 marker:text-tema-primary">
                {features.map((feature, index) => (
                    <li key={index}>{feature}</li>
                ))}
            </ul>
        </div>
        <Link
            to={linkTo}
            className="block w-full text-center mt-6 py-2 px-4 rounded-lg bg-tema-primary text-white font-bold transition-colors hover:bg-opacity-80"
        >
            {linkText}
        </Link>
    </div>
);

const HomePage = () => {
    const { isLoggedIn, userProfile, loadingProfile } = useAuth();

    const renderProfileCards = () => {
        if (isLoggedIn && loadingProfile) {
            return <p className="text-tema-text-muted dark:text-tema-text-muted-dark md:col-span-3 text-center">Carregando seu perfil...</p>;
        }

        if (isLoggedIn && userProfile) {
            // <<< NOVA LÓGICA BASEADA NAS FLAGS E PLANO >>>
            const cards = [];
            const isGratuito = userProfile.plano === 'GRATUITO';

            // Mostra módulos de Salão se isSalaoPro for true OU se o plano for Gratuito
            const mostrarModuloSalao = userProfile.isSalaoPro || isGratuito;
            // Mostra módulo Delivery se isDeliveryPro for true OU se o plano for Gratuito
            const mostrarModuloDelivery = userProfile.isDeliveryPro || isGratuito;

            if (mostrarModuloSalao) {
                cards.push(<ProfileCard key="garcom" title="App do Garçom" description="Gerencie mesas e faça pedidos." features={["Layout visual das mesas", "Cardápio completo", "Envio para a cozinha"]} icon={<LampiaoHatIcon />} linkText="Acessar Painel" linkTo="/mesas" />);
                cards.push(<ProfileCard key="caixa" title="App do Caixa" description="Feche contas e veja relatórios." features={["Controle de contas", "Processar pagamentos", "Relatório de vendas"]} icon={<CactusIcon />} linkText="Acessar Painel" linkTo="/caixa" />);
            }

            if (mostrarModuloDelivery) {
                cards.push(<ProfileCard key="delivery" title="Painel de Delivery" description="Receba e gerencie seus pedidos." features={["Painel Kanban", "Notificações de pedidos", "Controle de status"]} icon={<SmartphoneIcon />} linkText="Acessar Painel" linkTo="/delivery" />);
            }

            if (cards.length === 0) {
                 return <p className="text-tema-text-muted dark:text-tema-text-muted-dark md:col-span-3 text-center">Nenhum módulo ativo. Visite a seção Admin para gerenciar seus planos.</p>;
            }

            return <>{cards}</>;
        }

        // Conteúdo para o usuário deslogado (permanece o mesmo)
        return (
            <>
                <ProfileCard
                    title="Gestão de Salão"
                    description="Otimize o atendimento da sua equipe e evite erros em pedidos com um layout visual e intuitivo de mesas."
                    features={["Layout visual das mesas", "Cardápio completo organizado", "Envio rápido para cozinha", "Controle de contas"]}
                    icon={<LampiaoHatIcon />}
                    linkText="Conheça o Módulo"
                    linkTo="/#gestao-salao"
                />
                <ProfileCard
                    title="Cardápio Digital e Delivery"
                    description="Venda online sem comissões! Receba pedidos pelo seu próprio link, direto no painel do sistema."
                    features={["Link exclusivo para seu cardápio", "Receba pedidos ilimitados", "Fácil de compartilhar nas redes sociais", "Aumente suas vendas diretas"]}
                    icon={<SmartphoneIcon />}
                    linkText="Conheça o Módulo"
                    linkTo="/#delivery"
                />
                <ProfileCard
                    title="Frente de Caixa (PDV)"
                    description="Tenha controle total das suas finanças. Feche o caixa em segundos, processe pagamentos e acesse relatórios de vendas em tempo real."
                    features={["Visão geral das contas", "Múltiplas formas de pagamento", "Relatório de vendas diário", "Fechamento de caixa"]}
                    icon={<CactusIcon />}
                    linkText="Conheça o Módulo"
                    linkTo="/#caixa"
                />
            </>
        );
    };

    return (
        <div className="w-full p-8 max-w-6xl mx-auto space-y-12">
            {/* Seção Hero: Mensagem de Vendas */}
            <div className="text-center py-16">
                <h1 className="text-4xl md:text-5xl font-extrabold mb-4 text-tema-text dark:text-tema-text-dark">
                    Aumente as vendas, reduza a desordem.
                </h1>
                <p className="text-xl md:text-2xl text-tema-text-muted dark:text-tema-text-muted-dark max-w-3xl mx-auto">
                    A gestão completa para seu negócio. Do pedido na mesa ao delivery, tudo em um só lugar.
                </p>
                <div className="flex justify-center gap-4 mt-8">
                    <Link
                        to="/registrar"
                        className="bg-tema-primary text-white font-bold py-3 px-6 rounded-lg hover:bg-opacity-80 transition-colors text-lg"
                    >
                        Experimente Grátis
                    </Link>
                    <Link
                        to="/#saiba-mais"
                        className="bg-transparent text-tema-primary dark:text-tema-text-dark font-bold py-3 px-6 rounded-lg border border-tema-primary dark:border-tema-text-dark hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-lg"
                    >
                        Saiba Mais
                    </Link>
                </div>
            </div>

            {/* Seção de Prova Social (Exemplo) */}
            <div className="bg-white dark:bg-tema-surface-dark rounded-xl shadow-lg p-8 text-center border border-gray-200 dark:border-gray-700">
                <p className="italic text-lg text-tema-text dark:text-tema-text-dark">
                    "Desde que começamos a usar o Frevo na Mesa, o atendimento ficou mais rápido e nosso faturamento aumentou em 20%. Simples de usar e muito eficiente!"
                </p>
                <p className="font-bold text-tema-primary mt-4">
                    - José da Silva, Restaurante "Comida Boa"
                </p>
            </div>

            {/* Seção de Funcionalidades */}
            <div id="saiba-mais" className="text-center mb-12">
                <h2 className="text-3xl font-extrabold text-tema-text dark:text-tema-text-dark mb-4">
                    Recursos pensados para o seu sucesso
                </h2>
                <p className="text-lg text-tema-text-muted dark:text-tema-text-muted-dark">
                    Uma plataforma completa para simplificar sua operação diária.
                </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {renderProfileCards()}
            </div>

            {/* Seção de Chamada para Ação Final */}
            <div className="text-center pt-12 pb-8">
                <h2 className="text-3xl font-extrabold text-tema-text dark:text-tema-text-dark mb-4">
                    Pronto para começar?
                </h2>
                <p className="text-xl text-tema-text-muted dark:text-tema-text-muted-dark max-w-3xl mx-auto mb-6">
                    Experimente todas as funcionalidades do Frevo na Mesa por 7 dias. Sem compromisso e sem precisar de cartão de crédito.
                </p>
                <Link
                    to="/registrar"
                    className="bg-tema-primary text-white font-bold py-4 px-12 rounded-full hover:bg-opacity-80 transition-colors text-xl shadow-lg"
                >
                    Comece de Graça Agora
                </Link>
            </div>
        </div>
    );
};

export default HomePage;