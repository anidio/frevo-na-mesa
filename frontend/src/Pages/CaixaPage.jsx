...
// Componentes DashboardCard e FilaDeImpressao (sem alterações)
...
const CaixaPage = () => {
    ...
    // A página sempre será acessível se logado.

    const fetchData = async () => {
        try {
            // Busca sempre os dados, pois a página é acessível
            const promises = [
                apiClient.get('/api/caixa/dashboard'),
                apiClient.get('/api/mesas') // Sempre busca mesas para exibir, mesmo que limitadas
            ];

            // Busca pedidos pendentes apenas se a impressão de mesa estiver ativa no perfil
    ...
    useEffect(() => {
        ...
        if (!loadingProfile && userProfile) {
            fetchData();
            const intervalId = setInterval(fetchData, 15000); // Atualiza a cada 15s
    ...
    // Exibe loading enquanto perfil ou dados do caixa carregam
    ...
    // Se os dados não carregaram por outro motivo (ex: erro na API)
    if (!dashboardData) {
        return <div className="p-8 text-center text-tema-accent dark:text-red-400">Não foi possível carregar os dados do painel do caixa.</div>
    }

    // Filtra mesas (sempre executa)
    ...
    return (
        <div className="w-full p-4 md:p-8 max-w-7xl mx-auto space-y-8">
            ...
            {/* Seções de Mesas (Sempre renderiza, mostrando o conteúdo ou a mensagem de "Nenhuma mesa") */}
            <>
                {/* Fila de Impressão (Condicional à configuração do perfil) */}
