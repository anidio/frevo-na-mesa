// backend/src/main/java/br/com/frevonamesa/frevonamesa/service/RestauranteService.java
...
public class RestauranteService {
    ...
    public Restaurante cadastrar(RestauranteDTO restauranteDTO) {
        ...
        // Valores padrão para limites no cadastro (podem ser ajustados por planos depois)
        novoRestaurante.setLimiteMesas(10);
        novoRestaurante.setLimiteUsuarios(4);
        ...
        // Cria mesas iniciais apenas se o tipo permitir (MESAS ou MESAS_E_DELIVERY)
        if (restauranteSalvo.getTipo() == TipoEstabelecimento.APENAS_MESAS || restauranteSalvo.getTipo() == TipoEstabelecimento.MESAS_E_DELIVERY) {
            ...
            int limiteMesasIniciais = Math.min(restauranteSalvo.getLimiteMesas(), 5); // Cria no máximo 5 mesas iniciais, respeitando o limite do plano
            IntStream.rangeClosed(1, limiteMesasIniciais).forEach(numero -> {
            ...
            logger.debug("{} mesas iniciais criadas.", limiteMesasIniciais); // Log
        }
    ...
    public RestaurantePerfilDTO getPerfilLogado() {
        ...
        perfilDto.setCalculoHaversineAtivo(restaurante.isCalculoHaversineAtivo()); // Mapeamento corrigido
        ...
        perfilDto.setStripeConnectAccountId(restaurante.getStripeConnectAccountId()); // <-- ADIÇÃO DESTA LINHA
    ...
    public RestaurantePerfilDTO atualizarConfiguracoes(RestauranteSettingsDTO settingsDTO) {
        ...
        boolean valorRecebido = settingsDTO.isCalculoHaversineAtivo();
        ...
        restaurante.setCalculoHaversineAtivo(valorRecebido);
    ...
    public Map<String, Object> getStatusPlanoDetalhado() {
        ...
        int limitePedidosGratuito = 5; // Limite padrão
        ...
        // Correção aqui: A compensação deve subtrair do limite, não dos atuais
        ...
        // Nome mais claro para os pedidos compensados que ainda podem ser usados
        status.put("pedidosGratuitosRestantes", aplicaLimitePedidos ? Math.max(0, limitePedidosGratuito - pedidosAtuais) : null);
        status.put("limiteAtingido", limiteAtingido);
        status.put("stripeConnectAccountId", restaurante.getStripeConnectAccountId()); // <-- ADIÇÃO AQUI
    ...
    public CardapioPublicoDTO getCardapioPublico(Long restauranteId) {
        ...
        // <-- ADIÇÃO AQUI: Verifica se a conta connect ID não é nula nem vazia
        cardapioDTO.setPagamentoOnlineAtivo(restaurante.getStripeConnectAccountId() != null && !restaurante.getStripeConnectAccountId().isEmpty());
